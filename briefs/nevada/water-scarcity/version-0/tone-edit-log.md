...
Framing changes only. No facts, bill numbers, statistics, or citations added or removed. Section structure unchanged.
Disposition labels that match the dataset schema (`Failed=40`, `7 failed`, `Among 40 failed bills`, status `Failed` on
AB 265) were retained as factual stall/disposition language.
...
- [x] No forbidden advocacy phrases remain (`should`/`must`/`need to` directed at legislature; `refused`/`blocked`;
  certainty intensifiers; party insults; advocacy slogans)
