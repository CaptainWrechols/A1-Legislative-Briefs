# Explainer log — Growth, Infrastructure, and Roads citizen brief (audit aid)

Terms explained inline in the front brief or defined in the appended
glossaries, with where each explanation lives.

| Term | Where explained |
|---|---|
| Proviso / Part IB | Front brief budget callout ("one-year policy rules enacted inside the annual budget bill") + glossary |
| Penny tax / local option sales tax | Glossary + inline ("a new voter-approved penny tax counties can use...") |
| Transportation penny vs. capital project penny | Glossary (both entries) + local-funding spotlight |
| Impact fee | Glossary + inline ("a one-time charge on new development...") + developer-pays spotlight |
| Concurrency | Glossary + inline ("letting local governments require that roads and pipes keep pace before new development is approved") |
| Gas user fee / Act 40 of 2017 | Glossary + no-new-taxes spotlight ("phased the gas user fee up by twelve cents over six years") |
| Infrastructure maintenance fee | Glossary + inline ("the road-funding fee paid when a vehicle is first registered") |
| "C" funds / County Transportation Committees | Glossary + inline ("the county boards that spend gas-tax 'C' funds on local roads") |
| SCDOT Commission / Secretary of Transportation | Glossary + landscape/where-something-finished sections |
| Transportation Infrastructure Bank | Glossary |
| Design-build | Glossary + contractor spotlight ("choosing a contractor on qualifications first, then negotiating price") |
| Comprehensive plan / resiliency element | Glossary + planning spotlight |
| Home rule | Glossary |
| Annexation / donut hole | Glossary + planning spotlight |
| Road Buyback Program | Glossary + budget callout ("pays counties and cities to take back state roads, resurfaced first") |
| Committee outcomes without tallies | Process glossary + Appendix F |
| Roll call / vote pairs | Process glossary |
| First committee / money committees | Process glossary + political terrain |
| Conference committee | Process glossary + inline (S831 conference votes) |
| Joint resolution | Process glossary (gas suspensions, regulation approvals) |
| Placed on calendar without reference / recommitted / recalled | Process glossary + inline (S1069/S1070, H5071) |
| Ratified / Act number | Process glossary |
