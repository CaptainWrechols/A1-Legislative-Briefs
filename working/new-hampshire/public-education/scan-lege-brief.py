#!/usr/bin/env python3
"""Automated reviewer scans for the public-education lege brief.
...
BRIEF = Path("briefs/new-hampshire/public-education/citizen-v2/lege-brief.md")
...
ALLOWED_ADVICE = [
    ...
    "urged Congress",
    "districts must provide meals",
    "must adopt a student cell-phone",
    "rules every public school must meet",
    # process-glossary descriptions of legal requirements (identical wording
    # to the property-taxes lege brief's shared glossary)
...
for (y, b) in sorted(cited):
    if b in ("HB1", "HB2"):
        continue
    key = f"{y}:{b}"
    key2 = f"{y - 1}:{b}"  # biennium first-year record (e.g. SB33 2025-26)
...
DIVISION_WHITELIST = {
    (170, 185): "2025 HB675 reconsideration: 'Reconsider ITL (Rep. Weber): MF DV 170-185 01/08/2026 HJ 2' (division vote in the official docket, labeled as such in the brief)",
    (187, 193): "2024 SB523: 'Shall House Consider: MF DV 187-193 lacking necessary two-thirds vote 04/11/2024 HJ 11' (division vote in the official docket, labeled as such in the brief)",
...
for m in re.finditer(r"\b(\d{1,3})[–-](\d{1,3})\b", body):
    ...
    if prefix.endswith(":") or prefix.endswith("Rule "):
        continue  # chapter-section ranges (141:132–133) and Senate Rule 3-23
...
print(f"LEGE-BRIEF SCAN PASSED: no advice language; all {len(cited)} cited "
      ...
      f"({len(DIVISION_WHITELIST)} documented division/committee-vote exceptions).")
