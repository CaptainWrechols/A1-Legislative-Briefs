#!/usr/bin/env python3
"""Build appendices A-H for the NH public-education lege brief.
...
OUT = Path("briefs/new-hampshire/public-education/citizen-v2/appendices")
...
L = ["# Appendix A — Every bill in the set", "",
     "One row per bill found by the public-education search, 2020–2026. "
     "'Tier' marks how central the bill is: core (K-12 education policy), "
     "adjacent (touches the issue), or context (found by the keywords but not "
     "a K-12 education bill; kept for audit and excluded from headline counts). Results "
...
for b in bills:
    L.append(f"| {b['session_year']} | {b['identifier']} | {esc(b['plain_topic'])} "
             f"| {esc(b['theme'].replace('Context: not primarily a K-12 public-education bill', '—'))} "
...
L += ["", "The full machine-readable record — bills, votes, ballots, dockets, HB2 "
      ...
      "`sources/new-hampshire/public-education/` and "
      "`working/new-hampshire/public-education/`."]
...
L = ["# Appendix H — Inside the budget trailer (HB2)", "",
     "New Hampshire makes much of its education law inside HB2, the omnibus policy "
     "bill that accompanies each two-year budget. Every extracted section of "
     "the last three trailers was screened against the education terms "
